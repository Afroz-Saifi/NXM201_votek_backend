...
function jsonToArray(polls) {
  const convertedPolls = polls.map((poll) => {
    const convertedQuestions = Object.entries(poll.questions).map(
      ([questionId, question]) => {
        const convertedOptions = Object.entries(question.options).map(
          ([optionId, option]) => {
            return {
              optionId,
...
const decryptToken = (token) => {
  ...
  const decoded = jwt.verify(token, process.env.PRIMARY_SECRET_KEY);
  return decoded;
};
